...
import { ListarEtiquetas, CriarEtiqueta, AtualizarEtiqueta, DeletarEtiqueta } from 'src/service/tags'
...
export function useTag() {
  ...
  const atualizarEtiqueta = async (etiqueta) => {
    loading.value = true
    try {
      const { data } = await AtualizarEtiqueta(etiqueta.id, etiqueta)
  ...
  const salvarEtiqueta = async (etiqueta) => {
    if (etiqueta.id) {
      await atualizarEtiqueta(etiqueta)
    } else {
      await criarEtiqueta(etiqueta)
    }
  }
