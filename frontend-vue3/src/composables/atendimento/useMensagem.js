...
export function useMensagem() {
  ...
  const handleInputPaste = (e, ticketFocado) => {
    if (!ticketFocado?.id) return
    if (e.clipboardData.files[0]) {
      textChat.value = ''
      arquivos.value = [e.clipboardData.files[0]]
      abrirModalPreviewImagem.value = true
      urlMediaPreview.value = {
        title: `Enviar imagem para ${ticketFocado?.contact?.name}`,
  ...
  const handleStopRecordingAudio = async (ticketFocado, isScheduleDate) => {
    loading.value = true
    try {
