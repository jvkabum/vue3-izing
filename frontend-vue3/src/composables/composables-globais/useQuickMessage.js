...
export function useQuickMessage() {
  ...
  const criarMensagemRapida = async (mensagem) => {
    loading.value = true
    try {
      const { data } = await api.post('/quick-messages', mensagem)
  ...
  const atualizarMensagemRapida = async (mensagem) => {
    loading.value = true
    try {
      const { data } = await api.put(`/quick-messages/${mensagem.id}`, mensagem)
  ...
  const deletarMensagemRapida = async (mensagemId) => {
    try {
      ...
      loading.value = true
      await api.delete(`/quick-messages/${mensagemId}`)
  ...
  const salvarMensagemRapida = async (mensagem) => {
    if (mensagem.id) {
      return await atualizarMensagemRapida(mensagem)
    } else {
      return await criarMensagemRapida(mensagem)
    }
  }
