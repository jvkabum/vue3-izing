...
interface WhatsAppStoreGetters {
...
interface WhatsAppStoreActions {
  fetchSessions(): Promise<void>
  startSession(id: number): Promise<void>
  disconnectSession(id: number): Promise<void>
  setDefaultSession(id: number): Promise<void>
  showNotification(opts: NotifyOptions): void
  handleWhatsAppSession(data: WhatsAppResponse): void
}

export const useWhatsAppStore: StoreDefinition<
  'whatsapp',
  WhatsAppStoreState,
  WhatsAppStoreGetters,
  WhatsAppStoreActions
> = defineStore('whatsapp', {
  state: () => ({
    sessions: [],
    loading: false,
    error: null
